...
const dhKeyExchange = function (bls, skString, pkString) {
	...
	return Buffer.from(bls.publicKeyExport(sharedSec));
};

const bls_kE = function (bls, skString, pkString) {
	return dhKeyExchange(bls, skString, pkString).slice(0, 32);
};

const bls_kM = function (bls, skString, pkString) {
	return dhKeyExchange(bls, skString, pkString).slice(64, 128);
};
...
ECIES.prototype.encryptBLS = function(bls, message, ivbuf) {
	...
	const c = AESCBC.encryptCipherkey(message, bls_kE(bls, this._blsSecretKey, this._blsPublicKey), ivbuf.slice(0,16));
	const d = Hash.sha256hmac(c, bls_kM(bls, this._blsSecretKey, this._blsPublicKey));
	if(this.opts.shortTag) d = d.slice(0,4);
	let encbuf;
	if(this.opts.noKey) {
...
ECIES.prototype.decryptBLS = function(bls, encbuf) {
  ...
  */
	...
	var d2 = Hash.sha256hmac(c, bls_kM(bls, this._blsSecretKey, this._blsPublicKey));
	if(this.opts.shortTag) d2 = d2.slice(0,4);
	...
	return AESCBC.decryptCipherkey(c, bls_kE(bls, this._blsSecretKey, this._blsPublicKey));
};
