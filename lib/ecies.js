...
ECIES.prototype.getSharedSecret = function (bls, skString, pkString, cachedValue = 'sharedSec') {
  if (!this.cache.has(cachedValue)) {
    this.cache.set(cachedValue, dhKeyExchange(bls, skString, pkString));
  }
  return this.cache.get(cachedValue);
}

// Key derivation function for subkey kE
ECIES.prototype.getKDF_kE = function (bls, skString, pkString, cachedValue = 'kE') {
  if (!this.cache.has(cachedValue)) {
    this.cache.set(cachedValue, this.getSharedSecret(bls, skString, pkString).slice(0, 32));
  }
  return this.cache.get(cachedValue);
}

// Key derivation function for subkey kM
ECIES.prototype.getKDF_kM = function (bls, skString, pkString, cachedValue = 'kM') {
  if (!this.cache.has(cachedValue)) {
    this.cache.set(cachedValue, this.getSharedSecret(bls, skString, pkString).slice(64, 128));
  }
  return this.cache.get(cachedValue);
}
