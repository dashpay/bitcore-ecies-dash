...
ECIES.prototype.encryptBLS = function(bls, message, ivbuf) {
	...
	if (!Buffer.isBuffer(message)) message = new Buffer(message);
	console.log('this._blsSecretKey', this._blsSecretKey);
	const secArray = Buffer.from(this._blsSecretKey, 'hex');
	const sk = bls.secretKeyImport(secArray);
	const sharedSec = bls.publicKey();
	const pub = bls.publicKeyImport(Buffer.from(this._blsPublicKey, 'hex'));
	// Diffie-Helllman Key Exchange
	bls.dhKeyExchange(sharedSec, sk, pub);
	if (ivbuf === undefined) {
		ivbuf = Hash.sha256hmac(message, secArray).slice(0,16);
		console.log('ivbuf', ivbuf);
	}
	const c = AESCBC.encryptCipherkey(message, Buffer.from(bls.publicKeyExport(sharedSec)).slice(0,32), ivbuf.slice(0,16));
	const d = Hash.sha256hmac(c, Buffer.from(bls.publicKeyExport(sharedSec)).slice(64, 128));
	if(this.opts.shortTag) d = d.slice(0,4);
	let encbuf;
	if(this.opts.noKey) {
		encbuf = Buffer.concat([c, d]);
	} else {
		// for now don't prefix public key
		encbuf = Buffer.concat([c, d]);
	}
	console.log('encbuf', encbuf);
	return encbuf;
};
...
ECIES.prototype.decryptBLS = function(bls, encbuf) {
  ...
  */
	...
	var d2 = Hash.sha256hmac(c, Buffer.from(bls.publicKeyExport(sharedSec)).slice(64, 128));
	if(this.opts.shortTag) d2 = d2.slice(0,4);
	...
	return AESCBC.decryptCipherkey(c, Buffer.from(bls.publicKeyExport(sharedSec)).slice(0,32));
};
